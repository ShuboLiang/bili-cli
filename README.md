...
- `frames` —— 提取关键帧截图：优先用 Bilibili 雪碧图（秒级，免下载整个视频），降级到 ffmpeg 截帧。支持均匀取帧/按间隔/指定时间戳，输出图片 + JSON 清单供多模态 LLM 分析
...
# 关键帧截图（多模态 LLM 总结用）
bili-cli frames BV1xx411x7xx -n 8                        # 均匀取 8 帧（默认）
bili-cli frames BV1xx411x7xx --interval 30               # 每 30 秒一帧
bili-cli frames BV1xx411x7xx --at 10,60,120              # 指定时间戳
bili-cli frames BV1xx411x7xx --source storyboard         # 只用雪碧图（最快）
bili-cli frames BV1xx411x7xx --format png                # PNG 格式

# 机器可读 JSON（agent / 脚本）
bili-cli --json info BV1xx411x7xx          # 视频元数据 JSON
bili-cli --json transcript BV1xx411x7xx    # 逐字稿 JSON（含 fallback 降级提示）
bili-cli --json frames BV1xx411x7xx -n 8   # 帧清单 JSON（含文件路径+时间戳）
```
...
`frames --json` 返回图片清单，agent 可据此将图片喂给多模态 LLM：

```json
{
  "video": { "bvid": "...", "title": "...", "duration": 213 },
  "cid": 137649199,
  "source": "storyboard",
  "count": 8,
  "frames": [
    { "timestamp": 0.0, "file": "/path/to/frame_001.jpg" },
    { "timestamp": 30.4, "file": "/path/to/frame_002.jpg" },
    ...
  ]
}
```

**总结视频的完整 agent 流程**：`info --json` → `transcript --json`（文字底料）→ `frames --json -n 8`（视觉底料）→ 多模态 LLM 综合分析 → 带时间戳的结构化总结。无字幕时 transcript 返回降级指引，agent 自动切换到 `download --audio-only` + 语音转写。
...
src/
  ...
  commands/
    ...
    frames.rs    关键帧截图（雪碧图优先 + ffmpeg 降级）
```
